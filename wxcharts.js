...
function drawRadarLabel(angleList, radius, centerPosition, opts, config, context) {
    ...
    context.beginPath();
    context.setFontSize(config.fontSize);
    context.setFillStyle(radarOption.labelColor || '#000000');
...
function drawToolTipSplitLine(offsetX, opts, config, context) { //y标识线
    var startY = config.padding;
    var endY = opts.height - config.padding - config.xAxisHeight - config.legendHeight;
    
}

function drawToolTip(textList, offset, opts, config, context) {//显示指数
    var legendWidth = 0;
    var legendMarginRight = 8;
    var arrowWidth = 0;
...
function drawXAxis(categories, opts, config, context) {
    ...
    /**context.beginPath();
    context.setStrokeStyle(opts.xAxis.gridColor || "#cccccc");   //x轴下划线
    context.setLineWidth(1);
...
function drawRadarDataPoints(series, opts, config, context) {
    ...
    context.beginPath();
    context.setLineWidth(1);
    context.setStrokeStyle(radarOption.gridColor || "#ffffff");
